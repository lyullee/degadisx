...
def _require_legacy(backend: ThermoBackend | str | None) -> None:
    """Reject non-DEGADIS property routes in the public reimplementation."""
    if backend is not None and backend != "legacy":
        raise ValueError(
            "DEGADISx exposes only the DEGADIS 2.1 legacy thermodynamic route; "
            "non-legacy backends belong to separate research software."
        )
...
def run_source(
    ...
    *,
    ...
    thermo = case.make_thermo(backend="legacy")
...
def run_jet(
    ...
    *,
    ...
    th = Thermo(gas=gas, ambient=ambient, backend=LegacyBackend())
